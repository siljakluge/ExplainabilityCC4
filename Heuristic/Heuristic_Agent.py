...
class H_Agent():
    ...
    def _reset_agents(self):
        ...
        self.action_counter = np.array(
            [0, 0, 0, 0, 0, 0, 0])  # Monitor, Analyse, DeployDecoy, Remove, Restore, Block Traffic, Allow Traffic
    ...
    def _chosen_host_to_analyse(self):
        ...
        for host in self.analyse_host['Priority 3']:
            if 'server_host_0' in host:
                ...
                rest.append(host)
        assert len(servers_0) + len(servers) + len(rest) == len(
            self.analyse_host['Priority 3']), "Error in host classification"
        assert len(servers_0) != 0, "empty list error"
        assert len(rest) != 0, "empty list error"

        a = 1 / (self.priority_weights[0] * len(servers_0) + self.priority_weights[1] * len(servers) +
                 self.priority_weights[2] * len(rest))
        eps_server_0 = self.priority_weights[0] * a * len(servers_0)  # 1 server_0 (3 for HG)
        eps_server = self.priority_weights[1] * a * len(
            servers)  # 1-6 servers including server_0, but excluded in later choice
    ...
    def _interpret_message(self, message):
        origin_table = {
            "blue_agent_0": ["operational_zone_a_subnet", "restricted_zone_b_subnet", "operational_zone_b_subnet",
                             "public_access_zone_subnet"],
            "blue_agent_1": ["restricted_zone_a_subnet", "restricted_zone_b_subnet", "operational_zone_b_subnet",
                             "public_access_zone_subnet"],
            "blue_agent_2": ["restricted_zone_a_subnet", "operational_zone_a_subnet", "operational_zone_b_subnet",
                             "public_access_zone_subnet"],
            "blue_agent_3": ["restricted_zone_a_subnet", "operational_zone_a_subnet", "restricted_zone_b_subnet",
                             "public_access_zone_subnet"],
            "blue_agent_4": ["restricted_zone_a_subnet", "operational_zone_a_subnet", "restricted_zone_b_subnet",
                             "operational_zone_b_subnet"]}
        ...
        for i, message in enumerate(message):
            ...
            # remove host from list to be blocked if message indicates that host is secure again
                ...
                if origin_table[self.agent_name][i] in self.block_host and origin_table[self.agent_name][
                    i] not in self.mission_block_host:
    ...
    def get_action(self, obs, action_space):
        ...
        if len(self.observations) == 0:
            ...
            self.last_message = [np.array([False] * 8), np.array([False] * 8), np.array([False] * 8),
                                 np.array([False] * 8)]
        ...
        self.analyse_host['Priority 2'] = [item for item in self.analyse_host['Priority 2'] if
                                           item not in self.analyse_host['Priority 1']]
        ...
        # unblock if message indicates a safe host and host is still blocked
            ...
            if len(to_unblock) > 0:
                ...
                action = AllowTrafficZone(session=0, agent=self.agent_name, from_subnet=host_to_unblock,
                                          to_subnet=self.subnet)
                self.blocked_hosts.remove(host_to_unblock)
            else:
                host_to_block = to_block[0]
                action = BlockTrafficZone(session=0, agent=self.agent_name, from_subnet=host_to_block,
                                          to_subnet=self.subnet)
